...
print "Open dem. states  ($swinoD): $swistD\n" if $swinoD > 0;
print "Open rep. states  ($swinoR): $swistR\n" if $swinoR > 0;
print "Open toss-up states ($swinoT): $swistT\n" if $swinoT > 0;
printf "Democratic votes:   %3u\n", $demsafe if $demno > 0;
printf "Republican votes:   %3u\n", $repsafe if $repno > 0;
printf "Open votes:         %3u\n", $swingnum if $swino > 0;
printf "Open dem. votes:    %3u\n", $swingnumD if $swinoD > 0;
printf "Open rep. votes:    %3u\n", $swingnumR if $swinoR > 0;
printf "Open toss-up votes: %3u\n", $swingnumT if $swinoT > 0;
